...
const modes = [
  { id: "default", description: "Runtime default GC policy.", environment: {} },
  {
    id: "gogc-500",
...
for (const mode of modes) {
  mkdirSync(path.join(runDirectory, mode.id), { recursive: true });
}
...
runBalancedRounds("warmup", warmups, samples);
runBalancedRounds("measured", iterations, samples);

if (captureEvidence) {
  for (const [position, mode] of modes.entries()) {
    console.error(`[gc-policy] evidence ${mode.id}`);
    evidence.push(
      executeCompiler({
        mode,
        phase: "evidence",
        round: 1,
        position: position + 1,
        outputDirectory: path.join(runDirectory, mode.id),
        captureTrace: true,
...
const balancedPositions = iterations % modes.length === 0;
...
const report = {
  ...
  configuration: {
    ...
    iterations,
    captureEvidence,
    schedule: "deterministic rotating Latin order",
    balancedPositions,
  ...
  interpretation:
    ...
    parityPassed &&
    fastestValid.timeImprovementPercent >= 5
      ? "This target shows a parity-preserving, order-balanced single-project signal. It must reproduce on another real project before recommending a CLI GC policy."
      : "No order-balanced parity-preserving 5% single-project signal was established in this run."
};
...
if (!parityPassed || !balancedPositions) {
  process.exitCode = 1;
}

function runBalancedRounds(phase, rounds, destination) {
  for (let roundIndex = 0; roundIndex < rounds; roundIndex += 1) {
    const order = balancedOrder(modes, roundIndex);
    console.error(
      `\n[gc-policy] ${phase} round ${roundIndex + 1}/${rounds}: ${order
        .map((mode) => mode.id)
        .join(" -> ")}`
    );
...
function balancedOrder(values, roundIndex) {
  const cycle = Math.floor(roundIndex / values.length);
  const offset = roundIndex % values.length;
  const base = cycle % 2 === 0 ? [...values] : [...values].reverse();
...
function executeCompiler({
  ...
  executionIndex += 1;
  const runId = `${phase}-${String(round).padStart(2, "0")}-pos-${String(
    position
  ).padStart(2, "0")}`;
  ...
  const rssPath = path.join(
    os.tmpdir(),
    `tsgo-gc-rss-${process.pid}-${mode.id}-${executionIndex}.txt`
  );
...
function summarizeMode(mode, allSamples) {
  ...
  return {
    ...
    positions: measured.map((sample) => sample.position),
    statuses,
    diagnosticHashes,
...
function compareToDefault(summaries) {
  ...
  return summaries.map((summary) => {
    ...
    return {
      ...
      speedup: roundNumber(
        baseline.elapsedMs.median / summary.elapsedMs.median,
        3
      ),
...
function summarizeNumbers(values) {
  ...
  return {
    ...
    max: roundNumber(sorted.at(-1), 3),
    mean: roundNumber(mean, 3),
    median: roundNumber(percentile(sorted, 0.5), 3),
    p95: roundNumber(percentile(sorted, 0.95), 3),
    standardDeviation: roundNumber(standardDeviation, 3),
    coefficientOfVariationPercent:
      mean === 0 ? 0 : roundNumber((standardDeviation / mean) * 100, 3)
  };
}
...
function improvementPercent(baseline, candidate) {
  ...
  return roundNumber(((baseline - candidate) / baseline) * 100, 3);
}
...
function renderMarkdown(report) {
  const lines = [
    ...
    "",
    `Target: \`${escapeCell(report.preparation.target.id)}\` at \`${escapeCell(
      report.preparation.target.resolvedCommit
    )}\``,
    "",
    `TypeScript Go: \`${escapeCell(
      report.preparation.typescriptGo.resolvedCommit
    )}\``,
    "",
    `Schedule: **${report.configuration.schedule}**`,
    "",
    `Balanced positions: **${report.configuration.balancedPositions ? "yes" : "no"}**`,
    "",
    `Warm-up rounds: ${report.configuration.warmups}`,
    "",
    `Measured rounds: ${report.configuration.iterations}`,
    "",
    "## Results",
    "",
    "| Mode | Positions | Parity | Median ms | p95 ms | CV % | Time improvement | Speedup | Peak RSS KiB | RSS improvement | Median GC cycles | Median GC pause ms | Median allocated MiB |",
    "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"
  ];
  ...
  for (const summary of report.summaries) {
    ...
    lines.push(
      `| \`${escapeCell(summary.modeId)}\` | ${summary.positions.join(",")} | ${
        comparison.parity ? "yes" : "no"
      } | ${summary.elapsedMs.median} | ${summary.elapsedMs.p95} | ${
        summary.elapsedMs.coefficientOfVariationPercent
      } | ${comparison.timeImprovementPercent}% | ${comparison.speedup}x | ${
        summary.peakRssKb?.median ?? "n/a"
      } | ${
        comparison.peakRssImprovementPercent === null
          ? "n/a"
          : `${comparison.peakRssImprovementPercent}%`
      } | ${summary.runtime.numGC.median} | ${roundNumber(
        summary.runtime.pauseTotalNs.median / 1_000_000,
        3
      )} | ${roundNumber(
        summary.runtime.totalAlloc.median / 1024 / 1024,
        3
      )} |`
    );
  }
  ...
  lines.push(
    ...
    "",
    `Improvement versus default: **${
      report.fastestValidImprovementPercent ?? "n/a"
    }%**`,
    "",
    ...
    "",
    "- Each five-round cycle places every mode once in every execution position.",
    ...
    "- One project remains insufficient for an upstream recommendation.",
