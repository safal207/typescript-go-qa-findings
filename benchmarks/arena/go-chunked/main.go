...
func (a *Arena[T]) ensure(size int) {
	if size == 0 {
		return
	}
	if a.current != nil && len(a.current)+size <= cap(a.current) {
		return
	}
...
func (a *Arena[T]) NewSlice(size int) []T {
	...
	a.ensure(size)
	start := len(a.current)
	newLength := start + size
	result := a.current[start:newLength:newLength]
	a.current = a.current[:newLength]
	a.chunks[len(a.chunks)-1] = a.current
	return result
}
...
func main() {
	...
	report := Report{
		Implementation: "go-chunked-arena",
		Trace:          trace.ID,
